...
import com.neo.neoapp.UI.adapters.PeopleStateProfileCommentsAdapter;
...
public class PeopleStateProfileActivity extends NeoBasicActivity implements
	...
	private PeopleStateProfileCommentsAdapter mAdapter;
	...
	protected void initViews() {
		...
		mHeaderView = LayoutInflater.from(PeopleStateProfileActivity.this).inflate(
		...
		mHtvAge = (NeoBasicTextView) mHeaderView
	...
	private void init() {
		...
		mAdapter = new PeopleStateProfileCommentsAdapter(mApplication,
				PeopleStateProfileActivity.this, mComments);
	...
	private void initPopupWindow() {
		...
		mHeaderHeight = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, 48, getResources()
						.getDisplayMetrics());
		mPopupWindow = new PeopleStateListPopupWindow(this, mWidthAndHeight,
				mWidthAndHeight);
	...
	private void initHeaderView() {
		...
		if (!android.text.TextUtils.isEmpty(mPeople.getIndustry())) {
			...
			mIvIndustry.setImageBitmap(PhotoUtils.getIndustry(
					PeopleStateProfileActivity.this, mPeople.getIndustry()));
		} else {
			mIvIndustry.setVisibility(View.GONE);
		}
			...
			if (TextUtils.isEmpty(content)) {
				...
				FeedComment comment = new FeedComment("测试用户",
						"nearby_people_other", content, DateUtils.formatDate(
								PeopleStateProfileActivity.this,
	...
	private void startLoading() {
		...
		if (mIvLoading != null) {
			...
			mLoadingAnimation = AnimationUtils.loadAnimation(
					PeopleStateProfileActivity.this, R.anim.loading);
	...
	private void hideKeyBoard() {
		((InputMethodManager) getSystemService(INPUT_METHOD_SERVICE))
				.hideSoftInputFromWindow(PeopleStateProfileActivity.this
